...
def _render_marketing_plan_vs_actual(
    ...
    if not plan_df.empty and not actual_df.empty:
        merged = pd.merge(
            plan_df,
            actual_df,
            on=["Row", "JetLearn Deal Source", "Countries"],
            how="left"
        )

        merged["MTD Enrolments (Actual)"] = merged["MTD Enrolments (Actual)"].fillna(0).astype(float)
        merged["MTD Conv% (Actual)"] = merged["MTD Conv% (Actual)"].fillna(0).astype(float)
        ...
        try:
            ...
            chart_enrol = base.mark_bar().encode(
                column=alt.Column("Countries:N", title="Country Scope"),
                y=alt.Y("Planned Enrolments:Q", title="Enrolments"),
                tooltip=[
                    "JetLearn Deal Source",
                    "Countries",
                    "Planned Enrolments",
            ...
            chart_enrol_actual = base.mark_bar().encode(
                column=alt.Column("Countries:N"),
                y=alt.Y("MTD Enrolments (Actual):Q"),
                color=alt.value("#1f77b4")
            )

            st.altair_chart((chart_enrol + chart_enrol_actual).resolve_scale(y="shared"), use_container_width=True)
        except Exception:
            # If Altair fails, quietly skip chart
            pass
