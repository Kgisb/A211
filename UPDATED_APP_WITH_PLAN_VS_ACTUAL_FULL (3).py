...
def _render_marketing_plan_vs_actual(df):
    ...
    src_col = _col_exists(df, ["JetLearn Deal Source","Jetlearn Deal Source","Deal Source"])
    ctry_col = _col_exists(df, ["Country"])
    create_col = _col_exists(df, ["Create Date","Create date","Createdate"])
    pay_col = _col_exists(df, ["Payment Received Date","Payment Date","Enrolment Date","Enrollment Date","Paid On"])

    if src_col is None or ctry_col is None or create_col is None:
        st.error("Required columns missing to run Plan vs Actual (JetLearn Deal Source, Country, Create Date).")
        return

    # date parsing
    try:
        from __main__ import _dt_parse_dayfirst as _dtp
    except Exception:
        def _dtp(s):
    ...
    # drop invalid deals if present
    if "Deal Stage" in base.columns:
        mask_valid = base["Deal Stage"].astype(str).str.strip().str.lower() != "1.2 invalid deal(s)"
        base = base[mask_valid]

    create_dt = _dtp(base[create_col])
    pay_dt = _dtp(base[pay_col]) if pay_col else _pd.Series(_pd.NaT, index=base.index)

    unique_sources = sorted({str(s).strip() for s in base[src_col].dropna() if str(s).strip()})
    unique_countries = sorted({str(c).strip() for c in base[ctry_col].dropna() if str(c).strip()})

    st.markdown("### Plan vs Actual — by Source & Country")

    lb_months = st.selectbox(
    ...
    today = _pd.Timestamp.today().normalize()
    lb_start = today - _pd.DateOffset(months=lb_months)
    month_start = today.replace(day=1)

    default_rows = min(5, len(unique_sources)) or 3
    defaults = (unique_sources + ["Referral", "Webinar", "Performance Max"])[:default_rows]
    _init = _pd.DataFrame({
        "JetLearn Deal Source": defaults,
        "Countries": [["ALL"]] * default_rows,
        "Planned Deals": [0] * default_rows,
    })
    ...
    edited = st.data_editor(
        ...
        hide_index=True,
        column_config={
            "JetLearn Deal Source": st.column_config.SelectboxColumn(
                "JetLearn Deal Source",
                options=unique_sources,
                required=True,
            ),
            "Countries": st.column_config.MultiselectColumn(
                "Country(s)",
                options=["ALL"] + unique_countries,
                default=["ALL"],
                help="Type to search; use ALL for all countries",
            ),
            "Planned Deals": st.column_config.NumberColumn(
                "Planned Deals",
                min_value=0,
                step=1,
            ),
    ...
    if edited.empty:
        st.info("Add at least one row above to see Plan vs Actual.")
        return

    # 1) Plan based on lookback conversion
    plan_rows = []
    for _, r in edited.iterrows():
        src = str(r.get("JetLearn Deal Source") or "").strip()
        if not src:
            continue
        countries = r.get("Countries") or []
        if "ALL" in countries or len(countries) == 0:
            mask = (base[src_col] == src)
        else:
            mask = (base[src_col] == src) & (base[ctry_col].isin(countries))

        mask_lb = mask & (create_dt >= lb_start) & (create_dt <= today)
        deals_lb = int(mask_lb.sum())
        enrol_lb = int((mask_lb & pay_dt.notna()).sum()) if pay_col else 0
        conv = (enrol_lb / deals_lb) if deals_lb > 0 else 0.0

        planned_deals = int(r.get("Planned Deals") or 0)
        planned_enrol = planned_deals * conv

        plan_rows.append({
            "JetLearn Deal Source": src,
            "Countries": ", ".join(countries) if countries else "ALL",
            "Planned Deals": planned_deals,
            "Lookback Deals": deals_lb,
            "Lookback Enrolments": enrol_lb,
            "Historical Conversion %": round(conv * 100, 2),
            "Planned Enrolments (Projected)": round(planned_enrol, 2),
        })

    plan_df = _pd.DataFrame(plan_rows)
    st.markdown("#### 1. Plan — based on lookback conversion")
    st.dataframe(plan_df, use_container_width=True, hide_index=True)

    # 2) Actuals for current month (MTD)
    actual_rows = []
    for _, r in edited.iterrows():
        src = str(r.get("JetLearn Deal Source") or "").strip()
        if not src:
        ...
        mask_deals = mask & (create_dt >= month_start) & (create_dt <= today)
        deals = int(mask_deals.sum())

        if pay_col:
            mask_enrol = mask & pay_dt.notna() & (pay_dt >= month_start) & (pay_dt <= today)
            enrol = int(mask_enrol.sum())
        else:
            enrol = 0

        conv = (enrol / deals) if deals > 0 else 0.0

        actual_rows.append({
            "JetLearn Deal Source": src,
            "Countries": ", ".join(countries) if countries else "ALL",
            "Actual Deals (MTD)": deals,
            "Actual Enrolments (MTD)": enrol,
            "Actual Conversion % (MTD)": round(conv * 100, 2),
        })

    actual_df = _pd.DataFrame(actual_rows)
    st.markdown("#### 2. Actual — Current Month to Date")
    st.dataframe(actual_df, use_container_width=True, hide_index=True)

    # 3) Comparison chart
    comp = plan_df.merge(
        actual_df,
        on=["JetLearn Deal Source", "Countries"],
        how="outer",
    ).fillna(0)

    st.markdown("#### 3. Plan vs Actual — Visual Comparison")

    metric = st.radio(
        "Compare by",
        ["Conversion %", "Deals", "Enrolments"],
        index=0,
        horizontal=True,
        key="pva_metric",
    )
    ...
    if not comp.empty:
        if metric == "Deals":
            cols = ["Planned Deals", "Actual Deals (MTD)"]
            y_title = "Deals"
        elif metric == "Enrolments":
            cols = ["Planned Enrolments (Projected)", "Actual Enrolments (MTD)"]
            y_title = "Enrolments"
        else:
            cols = ["Historical Conversion %", "Actual Conversion % (MTD)"]
            y_title = "Conversion %"

        melt = comp.melt(
            id_vars=["JetLearn Deal Source", "Countries"],
            value_vars=cols,
            var_name="Type",
            value_name="Value",
        )
        melt["Label"] = melt["JetLearn Deal Source"].astype(str) + " | " + melt["Countries"].astype(str)

        chart = (
            alt.Chart(melt)
            .mark_bar()
            .encode(
                x=alt.X("Label:N", sort="-y", title="Source | Countries"),
                y=alt.Y("Value:Q", title=y_title),
                color=alt.Color("Type:N"),
                tooltip=["JetLearn Deal Source", "Countries", "Type", "Value"],
            )
        )
        st.altair_chart(chart, use_container_width=True)
    else:
        st.info("No data to plot for the selected configuration.")
...
# ---- Dispatcher for Marketing ▸ Plan vs Actual
if st.session_state.get("nav_master") == "Marketing" and st.session_state.get("nav_sub") == "Plan vs Actual":
    _df_any = None
    for _nm in ("df_f", "dff", "df"):
        _cand = globals().get(_nm, None)
        if _cand is not None:
