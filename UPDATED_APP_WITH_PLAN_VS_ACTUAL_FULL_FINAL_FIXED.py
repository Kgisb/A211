...
with st.sidebar:
    ...
    MASTER_SECTIONS = {
        ...
        "Marketing": [
        "Plan vs Actual","Referrals","HubSpot Deal Score tracker","Marketing Lead Performance & Requirement","Kids detail", "Deal Detail", "Sales Intern Funnel", "Intern vs AC", "Master analysis", "Referral Tracking", "Talk Time", "referral_Sibling", "Deal Score Trend", "Deal Score Threshold", "Invalid Deals", "Super report", "Marketing Plan"],
    }
    ...
    track = st.radio("Track", key="pva_track", ["Both", "AI Coding", "Math"], index=0)
...
def _render_performance_sales_activity(
    ...
    # --- Metric
    metric = st.selectbox("Metric", key="pva_metric", ["Trial Scheduled","Trial Rescheduled","Calibration Booked"], index=0, key=f"sa_metric_{uid}")
...
def _render_performance_activity_tracker(
    ...
    chart_type = st.selectbox("Chart type", key="pva_charttype", ["Bar","Line","Stacked Bar"], index=0, key="act_chart")
...
def _render_performance_deal_stage(
    ...
    if split_col:
        ...
        chart_type = st.selectbox("Chart type", key="pva_charttype", ["Bar","Line","Stacked Bar"], index=0, key="ds_chart")
        base = alt.Chart(chart_df).encode(
...
if view == "MIS":
    ...
    def _trend_and_analysis_tab():
        ...
        if (not referral_intent_col) or (referral_intent_col not in df_f.columns):
            ...
            melt_ref = grp_show.melt(
                ...
                var_name="Metric", key="pva_metric",
                value_name="Count"
            )
            chart_ref = (
                alt.Chart(melt_ref)
    ...
    if not monthly_sel.empty:
        ...
        bar_df["Metric", key="pva_metric"] = bar_df["Metric", key="pva_metric"].map({"CreatedCnt": "Deals Created", "PaidCnt": "Enrolments"})
    ...
    def _referrals_tab():
        ...
        if chart_mode == "Graph" and not day_join.empty:
            melt_day = day_join.melt(
                ...
                var_name="Metric", key="pva_metric", value_name="Count"
            )
            ch_day = (
                alt.Chart(melt_day)
                .mark_line(point=True)
        ...
        if choice_mom == "Graph" and not mom.empty:
            melt_mom = mom.melt(id_vars=["Month"], value_vars=["Referral Created","Referral Enrolments"], var_name="Metric", key="pva_metric", value_name="Count")
            ch_mom = (
                alt.Chart(melt_mom)
                .mark_bar(opacity=0.9)
                .encode(
        ...
        # ==========
            ...
            def _daily(by_cns=True):
                ...
                d1 = pd.DataFrame({
                    ...
                    "Metric", key="pva_metric": "Deals Created",
                })
                frames.append(d1)
                ...
                d2 = pd.DataFrame({
                    ...
                    "Metric", key="pva_metric": "Enrolments",
                })
                frames.append(d2)
                ...
                if m_first_eff is not None:
                    d3 = pd.DataFrame({
                        ...
                        "Metric", key="pva_metric": "First Cal Scheduled",
                    })
                    frames.append(d3)
                ...
                if m_resch_eff is not None:
                    d4 = pd.DataFrame({
                        ...
                        "Metric", key="pva_metric": "Cal Rescheduled",
                    })
                    frames.append(d4)
                ...
                if m_done_eff is not None:
                    d5 = pd.DataFrame({
                        ...
                        "Metric", key="pva_metric": "Cal Done",
                    })
                    frames.append(d5)
                    df_all = pd.concat([f for f in frames if not f.empty], ignore_index=True) if frames else pd.DataFrame(columns=["Date","Counsellor","Metric", key="pva_metric"])
                    df_all = df_all[df_all.get("Metric", key="pva_metric") != "Sales Generated (RIS)"]
                    if df_all.empty:
                        return df_all

                    df_all["Date"] = pd.to_datetime(df_all["Date"])
                    g = df_all.groupby(["Counsellor","Date","Metric", key="pva_metric"], observed=True).size().rename("Count").reset_index()
                    return g
            ...
            all_metrics = day["Metric", key="pva_metric"].unique().tolist()
            msel = st.multiselect("Metrics", options=all_metrics, default=all_metrics[:2], key="st_day_metrics")

            day_show = day[day["Metric", key="pva_metric"].isin(msel)].copy()
            ...
            # Table + Download
            st.dataframe(
                day_show.sort_values(["Counsellor","Date","Metric", key="pva_metric"]),
                use_container_width=True
            )
            st.download_button(
                "Download CSV — Day-wise Counsellor Metrics",
                day_show.sort_values(["Counsellor","Date","Metric", key="pva_metric"]).to_csv(index=False).encode("utf-8"),
        ...
        # ==========
            ...
            def _monthly():
                ...
                if m_done_eff is not None:
                    ...
                    frames.append(d5)
                    df_all = pd.concat([f for f in frames if not f.empty], ignore_index=True) if frames else pd.DataFrame(columns=["Month","Counsellor","Metric", key="pva_metric"])
                    df_all = df_all[df_all.get("Metric", key="pva_metric") != "Sales Generated (RIS)"]
                    if df_all.empty:
                        return df_all

                    g = df_all.groupby(["Counsellor","Month","Metric", key="pva_metric"], observed=True).size().rename("Count").reset_index()
                    return g
            ...
            # Pick metric(s) to plot
            all_metrics_m = mon["Metric", key="pva_metric"].unique().tolist()
            msel_m = st.multiselect("Metrics", options=all_metrics_m, default=all_metrics_m[:2], key="st_month_metrics")

            mon_show = mon[mon["Metric", key="pva_metric"].isin(msel_m)].copy()
            ...
            # Table + Download
            st.dataframe(
                mon_show.sort_values(["Counsellor","Month","Metric", key="pva_metric"]),
                use_container_width=True
            )
            st.download_button(
                "Download CSV — Monthly Counsellor Metrics",
                mon_show.sort_values(["Counsellor","Month","Metric", key="pva_metric"]).to_csv(index=False).encode("utf-8"),
...
if view == "Daily Business":
    ...
    with gp2:
        chart_type = st.selectbox("Chart type", key="pva_charttype", ["Bar", "Line", "Area (stack)", "Bar + Line (overlay)"], index=3,
    ...
    def _frame(date_series, mask, name):
        if mask is None or not mask.any():
            return pd.DataFrame(columns=["BucketKey","BucketLabel","Group","Metric", key="pva_metric","Count"])
        ...
        g["Metric", key="pva_metric"] = name
        return g[["BucketKey","BucketLabel","Group","Metric", key="pva_metric","Count"]]
    ...
    def _merge_counts(dfs):
        ...
        for dfi in dfs:
            ...
            piv = (dfi.pivot_table(index=["BucketKey","BucketLabel","Group"],
                                   columns="Metric", key="pva_metric", values="Count", aggfunc="sum")
                      .reset_index())
    ...
    for k in metrics_sel:
        if k not in wide.columns:
            wide[k] = np.nan
    plot_df = wide[keep_cols].melt(id_vars=["BucketKey","BucketLabel","Group"], var_name="Metric", key="pva_metric", value_name="Value")
            ...
            if color_field:
                ch = (
                    ...
                    .encode(
                        ...
                        column=alt.Column("Metric:N", title="Metric", key="pva_metric"),
                        tooltip=[tooltip_x,
                ...
                ch = (
                    ...
                    .encode(
                        x=x_enc, y=y_enc,
                        color=alt.Color("Metric:N", title="Metric", key="pva_metric", legend=alt.Legend(orient="bottom")),
    ...
    def _business_projection_tab():
        ...
        if proj_cur is not None:
            ...
            small_df = pd.DataFrame({
                "Metric", key="pva_metric": ["Actual MTD", "Projected Month-End"],
            ...
            ch2 = (
                ...
                .encode(
                    ...
                    tooltip=["Metric", key="pva_metric","Count"]
                )
...
if view == "Master Graph":
    def _master_graph_tab():
        ...
        if build_type == "Single metric":
            m1 = st.selectbox("Metric", key="pva_metric", metric_names, index=0, key="mg_m1")
...
def _render_performance_activity_concentration(
    ...
    chart_type = st.selectbox("Chart type", key="pva_charttype", ["Stacked Bar","Bar","Line"], index=0, key="ac_chart")
...
def _render_performance_referral_performance(
    ...
    if _bcol and _bcol in df.columns:
        ...
        metric = st.selectbox("Metric", key="pva_metric", ["Deals","Enrollments"], index=0, key="rp_metric")
        if metric == "Deals":
            if view_mode == "MTD":
    ...
    if ts.empty:
        ...
        ts_long = ts.melt(id_vars=["__ref","month"], value_vars=["Deals","Enrollments"], var_name="Metric", key="pva_metric", value_name="count")
...
def _render_performance_referral_no_referral(
    ...
    try:
        ...
        chart_df = pd.DataFrame([
            {"Metric", key="pva_metric":"Deals","Window":"A (Referral)","Value":resA["Deals"]},
            {"Metric", key="pva_metric":"Deals","Window":"B (Non-Referral)","Value":resB["Deals"]},
            {"Metric", key="pva_metric":"Enrollments","Window":"A (Referral)","Value":resA["Enrolments"]},
            {"Metric", key="pva_metric":"Enrollments","Window":"B (Non-Referral)","Value":resB["Enrolments"]},
            {"Metric", key="pva_metric":"Trial Scheduled","Window":"A (Referral)","Value":resA["FirstCal"]},
            {"Metric", key="pva_metric":"Trial Scheduled","Window":"B (Non-Referral)","Value":resB["FirstCal"]},
            {"Metric", key="pva_metric":"Trial Rescheduled","Window":"A (Referral)","Value":resA["CalResch"]},
            {"Metric", key="pva_metric":"Trial Rescheduled","Window":"B (Non-Referral)","Value":resB["CalResch"]},
            {"Metric", key="pva_metric":"Calibration Done","Window":"A (Referral)","Value":resA["CalDone"]},
            {"Metric", key="pva_metric":"Calibration Done","Window":"B (Non-Referral)","Value":resB["CalDone"]},
            {"Metric", key="pva_metric":"Referral generated (JetLearn Deal Source = referrals)","Window":"A (Referral)","Value":resA["ReferralGenerated"]},
            {"Metric", key="pva_metric":"Referral generated (JetLearn Deal Source = referrals)","Window":"B (Non-Referral)","Value":resB["ReferralGenerated"]},
            {"Metric", key="pva_metric":"Self-generated referrals (Referral Intent Source = Sales Generated)","Window":"A (Referral)","Value":resA["SelfGenReferrals"]},
            {"Metric", key="pva_metric":"Self-generated referrals (Referral Intent Source = Sales Generated)","Window":"B (Non-Referral)","Value":resB["SelfGenReferrals"]},
        ])
        st.altair_chart(
            alt.Chart(chart_df).mark_bar().encode(
                ...
                tooltip=["Metric", key="pva_metric","Window","Value"]
            ).properties(height=300),
            use_container_width=True
        )
...
def _render_funnel_trial_trend(
    ...
    ev = _pd.DataFrame(events, columns=["_when","Metric", key="pva_metric","_AC","_CNT","_SRC","_C"])
    ...
    if chart_type == "Stacked Bar":
        ...
        color_enc = _alt.Color("Metric:N", title="Metric", key="pva_metric")
    ...
    pivot = ser.pivot_table(index=time_col, columns="Metric", key="pva_metric", values="Count", aggfunc="sum", fill_value=0).reset_index()
...
def _render_marketing_deal_score_threshold(df):
    ...
    with c6:
        chart_type = st.selectbox("Chart type", key="pva_charttype", ["Bar","Stacked Bar","Line"], index=0)
...
def _render_marketing_marketing_plan(
    ...
    if show_chart and not agg_sorted.empty:
        a = agg_sorted.melt(id_vars=["Country"], value_vars=["Deals","Enrollments"], var_name="Metric", key="pva_metric", value_name="Count")
...
# === Integrated Plan vs Actual Pill ===
...
def _render_marketing_plan_vs_actual(df):
    ...
    with c1:
        lookback_months = st.selectbox(
            "Lookback period (for Plan conversion)", key="pva_lb",
    ...
    with c2:
        conv_mode = st.radio(
            "Conversion logic", key="pva_logic",
    ...
    with c3:
        ...
        actual_start = st.date_input("Actual period start", key="pva_start", value=default_start, key="pva_actual_start")
        actual_end = st.date_input("Actual period end", key="pva_end", value=today, key="pva_actual_end")
