...
def _render_marketing_plan_vs_actual(
    ...
    # Defensive import for relativedelta
    try:
    ...
    # Resolve core columns
    deal_source_col = _col_exists(df_loc, [
        "JetLearn Deal Source", "JetLearn deal source", "deal_source", "Jetlearn Deal Source"
    ])
    country_col = _col_exists(df_loc, [
        "Country", "country", "Student Country", "Contact: Country/Region"
    ])
    ...
    # Date columns
    if not create_col:
        create_col = _col_exists(df_loc, ["Create Date", "Create date", "Created Date"])
    if not pay_col:
        pay_col = _col_exists(df_loc, ["Payment Received Date", "Payment date", "Payment Date"])

    def _safe_parse(col_name):
        if not col_name or col_name not in df_loc.columns:
            return pd.Series(pd.NaT, index=df_loc.index)
        # Prefer global helper if present
        if "_dt_parse_dayfirst" in globals():
            return globals()["_dt_parse_dayfirst"](df_loc[col_name])
        return pd.to_datetime(df_loc[col_name], errors="coerce", dayfirst=True)

    _create = _safe_parse(create_col)
    _pay = _safe_parse(pay_col)

    # Extra metric underlying columns (if globals wired)
    if not first_cal_col:
        first_cal_col = globals().get("first_cal_sched_col")
    if not cal_resched_col:
        cal_resched_col = globals().get("cal_resched_col")
    if not cal_done_col:
        cal_done_col = globals().get("cal_done_col")

    _fc = _safe_parse(first_cal_col) if first_cal_col else pd.Series(pd.NaT, index=df_loc.index)
    _cr = _safe_parse(cal_resched_col) if cal_resched_col else pd.Series(pd.NaT, index=df_loc.index)
    _cd = _safe_parse(cal_done_col) if cal_done_col else pd.Series(pd.NaT, index=df_loc.index)

    # Available optional metrics for comparison
    available_extra = []
    if first_cal_col and first_cal_col in df_loc.columns:
        available_extra.append("First Calibration Count")
    if cal_resched_col and cal_resched_col in df_loc.columns:
        available_extra.append("Calib Reschedule Count")
    if cal_done_col and cal_done_col in df_loc.columns:
        available_extra.append("Calib Done Count")

    extra_selected = st.multiselect(
        "Optional indicators for comparison (besides Deals & Enrolments)",
    ...
    metric_defs = {
        "First Calibration Count": (
            "Planned First Cal Count",
            "MTD First Cal Count (Actual)",
            "Gap First Cal Count",
            _fc,
        ),
        "Calib Reschedule Count": (
            "Planned Calib Resch Count",
            "MTD Calib Resch Count (Actual)",
            "Gap Calib Resch Count",
            _cr,
        ),
        "Calib Done Count": (
            "Planned Calib Done Count",
            "MTD Calib Done Count (Actual)",
            "Gap Calib Done Count",
            _cd,
        ),
    }

    # All sources / countries for UI
    all_sources = sorted(x for x in df_loc[deal_source_col].dropna().astype(str).unique())
    all_countries = sorted(
        x for x in (
            df_loc[country_col].dropna().astype(str).unique()
            if country_col else []
        )
    )
    ...
    for i in range(int(rows_planned)):
        ...
        # Filter for lookback
        mask = df_loc[deal_source_col].astype(str).eq(src)
        if countries:
            mask &= df_loc[country_col].astype(str).isin(countries)

        hist_slice = df_loc.loc[
            mask &
            _create.between(hist_start, hist_end, inclusive="both")
        ]

        deals_hist = len(hist_slice)
        if deals_hist > 0:
            enroll_hist = int(hist_slice[ _pay.index ].loc[hist_slice.index].notna().sum()) if hasattr(_pay, "index") else int(hist_slice[pay_col].notna().sum()) if pay_col in hist_slice.columns else 0
            # Safer: recompute on pay series
            enroll_hist = int(
                _pay.loc[hist_slice.index].notna().sum()
            )
            conv_hist = enroll_hist / deals_hist
        else:
            enroll_hist = 0
            conv_hist = 0.0

        exp_enroll = deals_plan * conv_hist

        # Optional planned metrics
        extra_plan_vals = {}
        for label in extra_selected:
            plan_key, _, _, _ = metric_defs[label]
            extra_plan_vals[plan_key] = st.number_input(
                f"Row {i+1}: {plan_key}",
                min_value=0.0,
                value=0.0,
                step=1.0,
                key=f"pva_{plan_key.replace(' ', '_')}_{i}"
            )
        ...
        rec = {
            ...
            "Planned Enrolments": round(exp_enroll, 2),
        }
    ...
    if not plan_df.empty:
        ...
        st.info("Add at least one row with Deal Source and Planned Deals to see plan numbers.")

    # ---------- 2️⃣ Actuals ----------
    st.markdown("##### 2️⃣ Actual (MTD for current month)")

    mtd_start = pd.Timestamp(today.replace(day=1)).normalize()
    mtd_end = pd.Timestamp(today).normalize()

    # 2A. Pre-populated snapshot by Source x Country
    try:
        if country_col:
            df_mtd = df_loc.copy()
            df_mtd["is_deal_mtd"] = _create.between(mtd_start, mtd_end, inclusive="both")
            df_mtd["is_enrol_mtd"] = _pay.between(mtd_start, mtd_end, inclusive="both")

            agg_cols = {
                "is_deal_mtd": "MTD Deals",
                "is_enrol_mtd": "MTD Enrolments",
            }

            # Optional metrics aggregations for snapshot
            if "First Calibration Count" in extra_selected and not _fc.isna().all():
                df_mtd["is_fc_mtd"] = _fc.between(mtd_start, mtd_end, inclusive="both")
                agg_cols["is_fc_mtd"] = "MTD First Cal Count"
            if "Calib Reschedule Count" in extra_selected and not _cr.isna().all():
                df_mtd["is_cr_mtd"] = _cr.between(mtd_start, mtd_end, inclusive="both")
                agg_cols["is_cr_mtd"] = "MTD Calib Resch Count"
            if "Calib Done Count" in extra_selected and not _cd.isna().all():
                df_mtd["is_cd_mtd"] = _cd.between(mtd_start, mtd_end, inclusive="both")
                agg_cols["is_cd_mtd"] = "MTD Calib Done Count"

            grp = (
                df_mtd.groupby([deal_source_col, country_col], dropna=False)[list(agg_cols.keys())]
                .sum()
                .reset_index()
                .rename(columns={
                    deal_source_col: "JetLearn Deal Source",
                    country_col: "Country",
                    **agg_cols
                })
            )

            grp = grp[(grp["MTD Deals"] > 0) | (grp["MTD Enrolments"] > 0)]

            if not grp.empty:
                grp["MTD Conv%"] = (
                    (grp["MTD Enrolments"] / grp["MTD Deals"].where(grp["MTD Deals"] > 0))
                    .fillna(0) * 100
                )
                st.write("**Actuals Snapshot (auto, MTD)**")
                st.dataframe(
                    grp.sort_values(
                        ["JetLearn Deal Source", "Country", "MTD Enrolments"],
                ...
                st.info("No MTD activity yet for any Deal Source / Country.")
    except Exception:
        # Soft fail; snapshot is convenience only
        pass

    # 2B. Actuals aligned to Plan rows (for gaps)
    actual_records = []
    if not plan_df.empty:
        for _, r in plan_df.iterrows():
            src = r["JetLearn Deal Source"]
            planned_countries = str(r.get("Countries", "ALL"))
            countries = [] if planned_countries == "ALL" else [c.strip() for c in planned_countries.split(",") if c.strip()]
            ...
            deals_mtd = int(
                _create.loc[
                    mask & _create.between(mtd_start, mtd_end, inclusive="both")
                ].notna().sum()
            )
            enrol_mtd = int(
                _pay.loc[
                    mask & _pay.between(mtd_start, mtd_end, inclusive="both")
                ].notna().sum()
            )

            conv_mtd = (enrol_mtd / deals_mtd) if deals_mtd > 0 else 0.0

            # Actual countries contributing
            _mask_any = mask & (
                _create.between(mtd_start, mtd_end, inclusive="both") |
                _pay.between(mtd_start, mtd_end, inclusive="both")
            )
            _ctrs = (
                df_loc.loc[_mask_any, country_col]
                .dropna()
                .astype(str)
                .unique()
                if country_col else []
            )
            actual_countries_str = ", ".join(sorted(_ctrs)) if len(_ctrs) else "NA"

            rec = {
                "Row": int(r["Row"]),
                "JetLearn Deal Source": src,
                "Countries (Planned)": planned_countries,
                "Countries (Actual)": actual_countries_str,
                "MTD Deals (Actual)": deals_mtd,
                "MTD Enrolments (Actual)": enrol_mtd,
                "MTD Conv% (Actual)": round(conv_mtd * 100, 2),
            }

            # Extra actual metrics for this plan row
            for label in extra_selected:
                plan_key, actual_key, _, series = metric_defs[label]
                if series is not None:
                    cnt = int(
                        series.loc[
                            mask & series.between(mtd_start, mtd_end, inclusive="both")
                        ].notna().sum()
                    )
                else:
                    cnt = 0
    ...
    if not actual_df.empty:
        st.write("**Actuals vs Plan Rows (MTD)**")
        st.dataframe(actual_df, use_container_width=True, hide_index=True)
    elif not plan_df.empty:
        st.info("No matching MTD actuals found for the planned rows.")

    # ---------- 3️⃣ Gap & Comparison ----------
    st.markdown("##### 3️⃣ Gap & Comparison")

    if not plan_df.empty and not actual_df.empty:
        # Prepare plan side with explicit planned country col
        _plan = plan_df.copy()
        if "Countries (Planned)" not in _plan.columns and "Countries" in _plan.columns:
            _plan = _plan.rename(columns={"Countries": "Countries (Planned)"})
        ...
        # Ensure numeric
        for col in ["Planned Deals", "Planned Enrolments", "MTD Deals (Actual)", "MTD Enrolments (Actual)",
                    "Hist. Conv%", "MTD Conv% (Actual)"]:
            if col in merged.columns:
                merged[col] = merged[col].fillna(0).astype(float)

        # Core gaps
        merged["Gap Deals"] = merged.get("MTD Deals (Actual)", 0) - merged.get("Planned Deals", 0)
        merged["Gap Enrolments"] = merged.get("MTD Enrolments (Actual)", 0) - merged.get("Planned Enrolments", 0)
        merged["Gap Conv (pp)"] = merged.get("MTD Conv% (Actual)", 0) - merged.get("Hist. Conv%", 0)
        ...
        # Inject extra metric columns in a readable spot
        for label in extra_selected:
            plan_key, actual_key, gap_key, _ = metric_defs[label]
        ...
        # Simple comparison chart for enrolments
        try:
            import altair as alt
            chart_data = merged[["JetLearn Deal Source", "Planned Enrolments", "MTD Enrolments (Actual)"]].melt(
                "JetLearn Deal Source",
                var_name="Type",
            ...
            chart = (
                ...
                .encode(
                    ...
                    y=alt.Y("Enrolments:Q"),
                    color=alt.Color("Type:N"),
                    tooltip=["JetLearn Deal Source", "Type", "Enrolments"],
                )
            )
            st.altair_chart(chart, use_container_width=True)
        except Exception:
        ...
        st.info("Add plan rows above to see gap & comparison.")
    else:
        st.info("Actuals not available for the selected filters; ensure current month data exists.")
...
try:
    ...
    if _master_for_pva == "Marketing" and _view_for_pva == "Plan vs Actual":
        _render_marketing_plan_vs_actual(
            df_f=df if 'df' in globals() else df_f,
            create_col=create_col if 'create_col' in globals() else None,
            pay_col=pay_col if 'pay_col' in globals() else None,
            first_cal_col=first_cal_sched_col if 'first_cal_sched_col' in globals() else None,
            cal_resched_col=cal_resched_col if 'cal_resched_col' in globals() else None,
            cal_done_col=cal_done_col if 'cal_done_col' in globals() else None,
        )
except Exception as _e:
