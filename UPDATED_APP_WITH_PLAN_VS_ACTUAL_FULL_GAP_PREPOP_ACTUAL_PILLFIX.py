...
with st.sidebar:
    ...
    MASTER_SECTIONS = {
        ...
        "Marketing": ["Referrals", "HubSpot Deal Score tracker", "Marketing Funnel", "Sales Intern Funnel", "Intern vs AC", "Master analysis", "Referral Tracking", "Talk Time", "referral_Sibling", "Deal Score Trend", "Deal Score Threshold", "Invalid Deals", "Super report", "Marketing Plan", "Plan vs Actual"],
    }
...
def _render_marketing_plan_vs_actual(
    ...
    if df_f is None or getattr(df_f, "empty", True):
        st.warning("No data available to compute Plan vs Actual.")
        return

    # Resolve key columns
    deal_source_col = find_col(df_f, [
        "JetLearn Deal Source","JetLearn deal source",
        "Deal Source","Source","JetLearn_Deal_Source"
    ])
    country_col = find_col(df_f, [
        "Country","Deal Country","Country Name","Geo","Geography"
    ])
    _create = create_col or find_col(df_f, [
        "Create Date","Create date","Create_Date","Created At",
        "Created Date","Deal Create Date","Created On","Create_Date"
    ])
    _pay = pay_col or find_col(df_f, [
        "Payment Received Date","Payment received date","Payment_Received_Date",
        "Payment Date","Enrolment Date","Enrollment Date","Paid On"
    ])

    if not deal_source_col:
        st.error("Could not find 'JetLearn Deal Source' column.")
        return
    if not country_col:
        st.error("Could not find 'Country' column.")
        return
    if not _create or not _pay:
        st.error("Could not find required date columns for create / payment.")
        return

    # Coerce dates
    df_loc = df_f.copy()
    df_loc[_create] = coerce_datetime(df_loc[_create])
    df_loc[_pay] = coerce_datetime(df_loc[_pay])

    # Unique options
    all_sources = sorted([s for s in df_loc[deal_source_col].dropna().astype(str).unique() if s.strip() != ""])
    all_countries = sorted([c for c in df_loc[country_col].dropna().astype(str).unique() if c.strip() != ""])
    ...
    for i in range(int(rows_planned)):
        ...
        with c2:
            countries = st.multiselect(
                f"Row {i+1}: Country (type to search; leave blank for ALL)",
        ...
        if not src or deals_plan <= 0:
            # skip blank / zero-plan rows
            continue

        # Filter historical data for lookback window
        mask = df_loc[deal_source_col].astype(str).eq(src)

        if countries:
            mask &= df_loc[country_col].astype(str).isin(countries)

        # Use create date window for deal base; payment for enroll
        hist_slice = df_loc.loc[
            mask &
            df_loc[_create].between(hist_start, hist_end, inclusive="both")
        ]

        deals_hist = len(hist_slice)
        if deals_hist > 0:
            enroll_hist = int(hist_slice[_pay].notna().sum())
    ...
    if not plan_df.empty:
        ...
        st.info("Add at least one row with a Deal Source and Planned Deals to see plan numbers.")

    st.markdown("##### 2️⃣ Actual (MTD for current month)")

    # Current month to date window
    mtd_start = pd.Timestamp(today.replace(day=1)).normalize()
    mtd_end = pd.Timestamp(today).normalize()

    # 2A. Pre-populated Actual Snapshot (independent of plan)
    actual_snapshot_df = pd.DataFrame()
    try:
        if deal_source_col and country_col and not df_loc.empty:
            df_mtd = df_loc.copy()
            df_mtd["is_deal_mtd"] = df_mtd[_create].between(mtd_start, mtd_end, inclusive="both")
            df_mtd["is_enrol_mtd"] = df_mtd[_pay].between(mtd_start, mtd_end, inclusive="both")

            grp = (
                df_mtd.groupby([deal_source_col, country_col], dropna=False)[["is_deal_mtd", "is_enrol_mtd"]]
                .sum()
                .reset_index()
            )
            ...
            # Keep only rows with some activity
            grp = grp[(grp["MTD Deals"] > 0) | (grp["MTD Enrolments"] > 0)]

            if not grp.empty:
                grp["MTD Conv%"] = (grp["MTD Enrolments"] / grp["MTD Deals"].where(grp["MTD Deals"] > 0)).fillna(0) * 100
                actual_snapshot_df = grp
                st.write("**Actuals Snapshot (auto, MTD)**")
                st.dataframe(
                    actual_snapshot_df.sort_values(
            ...
            st.info("Actual snapshot unavailable — missing Deal Source / Country columns.")
    except Exception:
        # Failsafe: do not block the app if snapshot fails
        pass

    # 2B. Actuals aligned to Plan rows (used for Gap view)
    actual_records = []
    if not plan_df.empty:
        for _, r in plan_df.iterrows():
            src = r["JetLearn Deal Source"]
            countries = [] if r["Countries"] == "ALL" else [c.strip() for c in str(r["Countries"]).split(",") if c.strip()]
            ...
            deals_mtd = len(
                df_loc.loc[
                    mask &
                    df_loc[_create].between(mtd_start, mtd_end, inclusive="both")
                ]
            )
            # Enrolments (payment) MTD
            enrol_mtd = int(
                df_loc.loc[
                    mask &
                    df_loc[_pay].between(mtd_start, mtd_end, inclusive="both")
                ][_pay].notna().sum()
            )

            conv_mtd = (enrol_mtd / deals_mtd) if deals_mtd > 0 else 0.0

            # Derive actual countries contributing in MTD window for this source / planned filter
            _mask_mtd_any = mask & (
                df_loc[_create].between(mtd_start, mtd_end, inclusive="both") |
                df_loc[_pay].between(mtd_start, mtd_end, inclusive="both")
            )
            _actual_ctrs = (
                df_loc.loc[_mask_mtd_any, country_col]
                .dropna()
                .astype(str)
                .unique()
            )
            actual_countries_str = ", ".join(sorted(_actual_ctrs)) if len(_actual_ctrs) else "NA"

            actual_records.append({
                "Row": int(r["Row"]),
                "JetLearn Deal Source": src,
                "Countries (Planned)": r.get("Countries", "ALL"),
    ...
    if not actual_df.empty:
        ...
        st.info("Add plan rows above to see row-wise MTD actuals against plan.")
    else:
    ...
    if not plan_df.empty and not actual_df.empty:
        # Merge without using country to compute gaps at Row + Deal Source level
        _plan = plan_df.copy()
        if "Countries" in _plan.columns and "Countries (Planned)" not in _plan.columns:
        ...
        # Normalise country columns
        if "Countries (Actual)" not in merged.columns and "Countries_Actual" in merged.columns:
            merged = merged.rename(columns={"Countries_Actual": "Countries (Actual)"})

        # Fill numeric fields
        merged["Planned Deals"] = merged.get("Planned Deals", 0).astype(float)
        merged["MTD Deals (Actual)"] = merged.get("MTD Deals (Actual)", 0).fillna(0).astype(float)
        merged["Planned Enrolments"] = merged.get("Planned Enrolments", 0).astype(float)
        merged["MTD Enrolments (Actual)"] = merged.get("MTD Enrolments (Actual)", 0).fillna(0).astype(float)
        merged["Hist. Conv%"] = merged.get("Hist. Conv%", 0).astype(float)
        merged["MTD Conv% (Actual)"] = merged.get("MTD Conv% (Actual)", 0).fillna(0).astype(float)

        # Compute gaps
        merged["Gap Deals"] = merged["MTD Deals (Actual)"] - merged["Planned Deals"]
        merged["Gap Enrolments"] = merged["MTD Enrolments (Actual)"] - merged["Planned Enrolments"]
        merged["Gap Conv (pp)"] = merged["MTD Conv% (Actual)"] - merged["Hist. Conv%"]
        ...
        # Insert country info (not used for matching, only for context)
        for col in ["Countries (Planned)", "Countries (Actual)"]:
            if col in merged.columns and col not in display_cols:
        ...
        # Comparative chart: Planned vs Actual Enrolments by Deal Source
        try:
            import altair as alt
