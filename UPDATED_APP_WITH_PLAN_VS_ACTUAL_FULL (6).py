...
try:
    if master == "Marketing" and st.session_state.get('nav_sub') == "Plan vs Actual":
        _df_any = None
        for _nm in ("dff","df_f","df"):
            _cand = globals().get(_nm, None)
            if _cand is not None:
                ...
                break
        if _df_any is not None:
            _render_marketing_plan_vs_actual(_df_any)
        else:
            st.error("No dataframe available to render Plan vs Actual.")
except Exception as _e:
    import traceback, streamlit as st
    st.error(f"Plan vs Actual failed: {_e}")
    st.code("".join(traceback.format_exc())[-8000:])
...
def _render_marketing_plan_vs_actual(
    ...
    """
    Plan vs Actual:
    - User defines planned deals per JetLearn Deal Source & Countries.
    - System derives historical conversion% using lookback window (months).
    - Planned enrollments = Planned Deals × Historical Conversion%.
    - Actual shows current month-to-date deals & enrollments.
    - Comparative chart for Plan vs Actual on % or counts.
    """
    import streamlit as st
    import pandas as pd
    import numpy as np
    ...
    if df_f is None or getattr(df_f, "empty", True):
        st.info("No data available for Plan vs Actual.")
        return
    ...
    _create = _pick(df_f, create_col, "Create Date","Created Date","Deal Create Date","CreateDate")
    _pay    = _pick(df_f, pay_col, "Payment Received Date","Payment Date","Paid On","Enrollment Date")
    _src    = _pick(df_f, source_col, "JetLearn Deal Source","Jetlearn Deal source","JetLearn Source","Source")
    _ctry   = _pick(df_f, country_col, "Country","Customer Country","Geo")

    if not _create or not _src:
        st.error("Required columns missing: Create Date / JetLearn Deal Source.")
        return

    df = df_f.copy()

    # Parse key dates
    for col in [_create, _pay]:
        if col and col in df.columns:
    ...
    # Canonical lists
    all_sources = sorted([s for s in df[_src].dropna().astype(str).unique()])
    all_countries = sorted([c for c in df[_ctry].dropna().astype(str).unique()]) if _ctry else []

    st.subheader("Marketing — Plan vs Actual")

    st.markdown("**Step 1: Define your plan**")
    with st.expander("Planning grid", expanded=True):
        col_top = st.columns([2,2])
        with col_top[0]:
            default_rows = 3
            num_rows = st.number_input("Number of plan rows", 1, 25, default_rows, step=1, key="plan_vs_actual_rows")
        with col_top[1]:
            st.caption("For each row choose Deal Source, Countries, Lookback window and Planned deals.")

        lookback_options = [1,2,3,6,9,12,18,24]
        ...
        for i in range(int(num_rows)):
            ...
            with c2:
                if all_countries:
                    ...
                    countries = ["All"]
                    st.text_input(
                        f"Countries (text) #{i+1}",
                        value="All",
                        key=f"pva_ctry_txt_{i}"
                    )
            with c3:
                lb = st.selectbox(
                    f"Lookback (months) #{i+1}",
                    options=lookback_options,
                    index=2,  # default 3 months
                    key=f"pva_lb_{i}"
                )
            with c4:
                planned_deals = st.number_input(
                    f"Planned deals #{i+1}",
                    min_value=0,
                    value=0,
                    step=1,
            ...
            if src:
                plan_rows.append({
                    ...
                    "Countries": countries,
                    "Lookback Months": int(lb),
                    "Planned Deals": int(planned_deals),
                })

    if not plan_rows:
        st.info("Add at least one valid planning row above.")
        return

    # --- Step 2: Compute plan metrics from history ---
    result_records = []
    month_start = date(today.year, today.month, 1)

    for row in plan_rows:
        src = row["JetLearn Deal Source"]
        countries = row["Countries"]
        lb_months = int(row["Lookback Months"])
        planned_deals = int(row["Planned Deals"])

        # Historical window
        start_hist = month_start - relativedelta(months=lb_months)
        # Use created-date-based funnel in lookback window
        hist_mask = (df[_create].dt.date >= start_hist) & (df[_create].dt.date < month_start)
        hist_mask &= df[_src].astype(str).eq(src)

        # Country filter for history
        if _ctry and countries and "All" not in countries:
            hist_mask &= df[_ctry].astype(str).isin(countries)

        hist_df = df.loc[hist_mask].copy()
        hist_deals = len(hist_df)
        hist_enrol = hist_df[_pay].notna().sum() if _pay else 0
        hist_conv = (hist_enrol / hist_deals * 100.0) if hist_deals > 0 else 0.0

        planned_enrol = planned_deals * hist_conv / 100.0

        # Actuals: current month-to-date
        m_mask = (df[_create].dt.date >= month_start) & (df[_create].dt.date <= today)
        m_mask &= df[_src].astype(str).eq(src)
        if _ctry and countries and "All" not in countries:
            m_mask &= df[_ctry].astype(str).isin(countries)

        m_df = df.loc[m_mask].copy()
        actual_deals = len(m_df)
        actual_enrol = m_df[_pay].notna().sum() if _pay else 0
        actual_conv = (actual_enrol / actual_deals * 100.0) if actual_deals > 0 else 0.0

        countries_disp = "All" if (not countries or "All" in countries) else ", ".join(sorted(set(countries)))

        result_records.append({
            "JetLearn Deal Source": src,
            "Countries": countries_disp,
            "Lookback Months": lb_months,
            "Hist Deals (LB)": hist_deals,
            "Hist Enrol (LB)": hist_enrol,
            "Hist Conv% (LB)": round(hist_conv, 2),
            "Planned Deals": planned_deals,
            "Planned Enrol": round(planned_enrol, 2),
            "Actual Deals (MTD)": actual_deals,
            "Actual Enrol (MTD)": actual_enrol,
            "Actual Conv% (MTD)": round(actual_conv, 2),
        })

    res_df = pd.DataFrame(result_records)

    st.markdown("**Step 2: Plan vs Actual table**")
    st.dataframe(res_df, use_container_width=True)

    # --- Step 3: Comparative chart ---
    st.markdown("**Step 3: Comparative chart**")
    metric = st.radio(
        "Compare by",
        ["Conversion %", "Enrollments", "Deals"],
        index=0,
        horizontal=True,
        key="pva_metric"
    )
    ...
    for _, r in res_df.iterrows():
        label = f"{r['JetLearn Deal Source']} | {r['Countries']}"
        if metric == "Conversion %":
            chart_rows.append({"Group": label, "Type": "Planned", "Value": r["Hist Conv% (LB)"]})
            chart_rows.append({"Group": label, "Type": "Actual", "Value": r["Actual Conv% (MTD)"]})
        elif metric == "Enrollments":
            chart_rows.append({"Group": label, "Type": "Planned", "Value": r["Planned Enrol"]})
            chart_rows.append({"Group": label, "Type": "Actual", "Value": r["Actual Enrol (MTD)"]})
        else:  # Deals
            chart_rows.append({"Group": label, "Type": "Planned", "Value": r["Planned Deals"]})
            chart_rows.append({"Group": label, "Type": "Actual", "Value": r["Actual Deals (MTD)"]})
    ...
    if chart_rows:
        ...
        chart = (
            ...
            .mark_bar()
            .encode(
                x=alt.X("Group:N", sort=None, title="Source | Countries"),
        ...
        st.info("No data to plot for the chosen setup.")
