...
def _render_marketing_plan_vs_actual(df):
    ...
    # Local helpers
    def _find_col_local(_df, candidates):
    ...
    src_col = _find_col_local(df, ["JetLearn Deal Source", "Jetlearn Deal Source", "Deal Source", "Lead Source", "Source"])
    country_col = _find_col_local(df, ["Country", "Country Name", "Geo", "Region"])
    create_col = _find_col_local(df, ["Create Date", "Created Date", "Deal create date", "Created On"])
    pay_col = _find_col_local(df, ["Payment Received Date", "Payment Received date", "Enrolment Date", "Deal Won Date"])

    if not src_col or not country_col or not create_col or not pay_col:
        st.warning("Required columns missing for Plan vs Actual (JetLearn Deal Source / Country / Create / Payment Received).")
        return
    ...
    st.subheader("Plan vs Actual — Deal Source × Country")

    # Scope selectors
    all_sources = sorted(src_series.dropna().unique().tolist())
    all_countries = sorted(country_series.dropna().unique().tolist())
    ...
    if not selected_sources or not selected_countries:
        st.info("Select at least one Deal Source and one Country.")
        return

    # Conversion basis
    conv_mode = st.radio(
        "Conversion basis",
        ["MTD (Deal & Enrol in window)", "Cohort (Deals in window, Enrol anytime)"],
        index=0,
        horizontal=True,
        key="pva2_mode"
    )
    ...
    # Window pickers
    def _pick_window(label_prefix, key_prefix):
        import datetime as _dt
        today = _dt.date.today()
        opt = st.selectbox(
            f"{label_prefix} window",
            ["This Month (MTD)", "Last 30 Days", "Custom"],
            key=f"{key_prefix}_type"
        )
        if opt == "This Month (MTD)":
            start_d = today.replace(day=1)
            end_d = today
        elif opt == "Last 30 Days":
            start_d = today - _dt.timedelta(days=30)
            end_d = today
        else:
            c_start, c_end = st.columns(2)
            with c_start:
                start_d = st.date_input(f"{label_prefix} start", value=today.replace(day=1), key=f"{key_prefix}_start")
            with c_end:
                end_d = st.date_input(f"{label_prefix} end", value=today, key=f"{key_prefix}_end")
            if start_d > end_d:
                st.error(f"{label_prefix}: start date cannot be after end date.")
        start_ts = pd.to_datetime(start_d)
        end_ts = pd.to_datetime(end_d) + pd.Timedelta(days=1) - pd.Timedelta(seconds=1)
        return start_ts, end_ts

    col_lb, col_ac = st.columns(2)
    with col_lb:
        lb_start, lb_end = _pick_window("Lookback (for planned conversion)", "pva2_lb")
    with col_ac:
        ac_start, ac_end = _pick_window("Actual (current performance)", "pva2_ac")
    ...
    plan_edit = st.data_editor(
        base_df,
        key="pva2_plan_editor",
        use_container_width=True,
        num_rows="dynamic"
    )

    def _parse_countries(val):
        if not isinstance(val, str) or val.strip() == "" or val.strip().lower() == "all":
            return None
        parts = [p.strip() for p in val.split(",") if p.strip()]
        return parts or None

    def _deals_enrol_for(src_val, countries_val, start_ts, end_ts, mode):
        m = (src_series == str(src_val))

        row_countries = _parse_countries(countries_val)
        if row_countries is None:
            m &= country_series.isin(selected_countries)
        else:
            m &= country_series.isin(row_countries)
    ...
    for _, r in plan_edit.iterrows():
        src_val = r.get("JetLearn Deal Source")
        if pd.isna(src_val) or str(src_val).strip() == "":
            continue
        countries_val = r.get("Countries (comma-separated or 'All')", "All")
        try:
            plan_deals = float(r.get("Plan Deals", 0) or 0)
        except Exception:
            plan_deals = 0.0

        # Lookback-based planned conversion
        lb_deals, lb_enrol = _deals_enrol_for(src_val, countries_val, lb_start, lb_end, conv_mode)
        plan_conv = (lb_enrol / lb_deals) * 100.0 if lb_deals > 0 else 0.0
        planned_enrol = plan_deals * (plan_conv / 100.0)

        # Actuals
        ac_deals, ac_enrol = _deals_enrol_for(src_val, countries_val, ac_start, ac_end, conv_mode)
        ac_conv = (ac_enrol / ac_deals) * 100.0 if ac_deals > 0 else 0.0

        records.append({
            "JetLearn Deal Source": src_val,
            ...
            "Planned Enrollments": round(planned_enrol, 2),
            "Actual Deals": ac_deals,
            "Actual Enrollments": ac_enrol,
            "Actual Conversion %": round(ac_conv, 2),
            "Gap Enrollments (Actual - Planned)": round(ac_enrol - planned_enrol, 2),
            "Gap Conversion % (Actual - Planned)": round(ac_conv - plan_conv, 2),
        })

    if not records:
        st.info("Add at least one valid row in the plan table.")
        return

    out_df = pd.DataFrame(records)
    st.dataframe(out_df, use_container_width=True)

    # Visualization: Planned vs Actual Conversion
    try:
        chart_df = out_df.melt(
            id_vars=["JetLearn Deal Source", "Countries"],
            value_vars=["Planned Conversion % (from lookback)", "Actual Conversion %"],
            var_name="Type",
        ...
        chart = (
            ...
            .encode(
                ...
                y=alt.Y("Conversion %:Q"),
                color=alt.Color("Type:N", title=""),
                column=alt.Column("Countries:N", title="Countries"),
                tooltip=["JetLearn Deal Source", "Countries", "Type", "Conversion %"]
            )
            .properties(height=260)
        )
        st.altair_chart(chart, use_container_width=True)
    ...
    st.download_button(
        "Download CSV — Plan vs Actual (2.0)",
        data=out_df.to_csv(index=False).encode("utf-8"),
        file_name="plan_vs_actual_2_0.csv",
        mime="text/csv",
        key="pva2_dl"
    )

# Dispatcher hook (uses the latest definition above)
try:
    ...
    st.code("".join(_tb.format_exc())[-8000:])
