...
def _render_marketing_plan_vs_actual(df):
    ...
    src_col = _find_col(df, ["JetLearn Deal Source", "Jetlearn Deal Source", "Lead Source", "Source"])
    country_col = _find_col(df, ["Country", "Country Name", "Geo", "Region"])
    first_cal_col = _find_col(df, ["First Calibration Scheduled Date", "First Trial Scheduled Date", "First Trial Date"])
    resched_col = _find_col(df, ["Calibration Rescheduled Date", "Trial Rescheduled Date"])
    done_col = _find_col(df, ["Calibration Done Date", "Trial Done Date"])
    pay_col = _find_col(df, ["Payment Received Date", "Enrolment Date", "Deal Won Date"])

    if not src_col or not country_col:
        st.warning("Required columns missing (JetLearn Deal Source / Country).")
        return

    plan_data = pd.DataFrame({
        "JetLearn Deal Source": ["Referral", "Facebook Ads", "Google Ads", "LinkedIn"],
        "Country": ["India", "UK", "Netherlands", "USA"],
        "Planned Enrollments": [50, 40, 30, 20],
        "Planned Conversion %": [30, 25, 20, 35],
    })

    st.markdown("### Pre-fed Plan Data (Editable)")
    edited_plan = st.data_editor(plan_data, key="pva_edit", use_container_width=True, num_rows="dynamic")

    c1, c2, c3 = st.columns(3)
    with c1:
        sel_country = st.selectbox("Select Country", ["All"] + sorted(df[country_col].dropna().unique().tolist()), index=0)
    with c2:
        sel_source = st.selectbox("Select Deal Source", ["All"] + sorted(df[src_col].dropna().unique().tolist()), index=0)
    with c3:
        mode = st.radio("Mode", ["Entity Level", "Cohort Level"], horizontal=True)

    metric_type = st.selectbox(
        "Metric type for comparison",
        ["First Calibration Scheduled Date", "Calibration Rescheduled Date", "Calibration Done Date"],
        index=0
    )

    metric_col = first_cal_col if "First" in metric_type else resched_col if "Resched" in metric_type else done_col
    metric_dt = _to_dt(df[metric_col]) if metric_col else pd.Series(pd.NaT, index=df.index)
    pay_dt = _to_dt(df[pay_col]) if pay_col else pd.Series(pd.NaT, index=df.index)

    today = pd.Timestamp.today().normalize()
    start_date = st.date_input("Lookback Start Date", today - pd.Timedelta(days=30))
    end_date = st.date_input("End Date", today)

    start_ts = pd.to_datetime(start_date)
    end_ts = pd.to_datetime(end_date) + pd.Timedelta(days=1) - pd.Timedelta(seconds=1)

    mask = metric_dt.between(start_ts, end_ts)
    if sel_country != "All":
        mask &= df[country_col].astype(str) == sel_country
    if sel_source != "All":
        mask &= df[src_col].astype(str) == sel_source

    dfa = df[mask].copy()
    dfa["Enrollments"] = pay_dt[mask].notna().astype(int)
    dfa["Metric Done"] = metric_dt[mask].notna().astype(int)

    agg = dfa.groupby([src_col, country_col], dropna=False).agg(
        Deals=(src_col, "count"),
        Metric_Done=("Metric Done", "sum"),
        Enrollments=("Enrollments", "sum"),
    ).reset_index()

    agg["Conversion %"] = np.where(agg["Deals"] > 0, (agg["Enrollments"] / agg["Deals"]) * 100, 0).round(2)
    agg["As of Date"] = today.date()

    merged = pd.merge(edited_plan, agg, how="outer", left_on=["JetLearn Deal Source", "Country"],
                      right_on=[src_col, country_col])
    merged["Gap (Enrollments)"] = merged["Enrollments"].fillna(0) - merged["Planned Enrollments"].fillna(0)
    merged["Gap (%)"] = merged["Conversion %"].fillna(0) - merged["Planned Conversion %"].fillna(0)

    st.markdown("### Plan vs Actual Summary")
    st.dataframe(merged[[
        "JetLearn Deal Source", "Country", "Planned Enrollments", "Enrollments",
        "Gap (Enrollments)", "Planned Conversion %", "Conversion %", "Gap (%)", "As of Date"
    ]], use_container_width=True)

    st.download_button(
        "Download CSV — Plan vs Actual",
        data=merged.to_csv(index=False).encode("utf-8"),
        file_name="plan_vs_actual.csv",
        mime="text/csv",
        key="pva_dl"
    )
