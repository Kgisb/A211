...
def _render_marketing_plan_vs_actual(
    ...
    for i in range(int(rows_planned)):
        ...
        if deals_hist > 0:
            # Enrolments in lookback window based on payment dates
            enroll_hist = int(_pay.loc[hist_slice.index].notna().sum())
        ...
        for label in extra_selected:
            ...
            if series is not None:
                # Historical count for this metric within lookback window
                try:
                    hist_events = int(
                        series.loc[
                            mask & series.between(hist_start, hist_end, inclusive="both")
                        ].notna().sum()
                    )
                except Exception:
                    # Fallback: safe 0 if anything goes off
                    hist_events = 0
                ratio = (hist_events / deals_hist) if deals_hist > 0 else 0.0
                extra_plan_vals[plan_key] = round(deals_plan * ratio, 2)
            else:
                extra_plan_vals[plan_key] = 0.0
