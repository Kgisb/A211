...
# User sees MTD / Cohort. We FLIP the internal logic as requested:
    ...
    if not plan_df.empty and not actual_df.empty:
        ...
        try:
            # Define all supported metrics (labels → (plan_col, actual_col))
            metric_map = {
            ...
            # If required columns are missing for chosen metric, show info instead of failing
            if plan_col not in merged.columns or act_col not in merged.columns:
                st.info(f"Selected metric '{metric_label}' is not available in the current view.")
            else:
