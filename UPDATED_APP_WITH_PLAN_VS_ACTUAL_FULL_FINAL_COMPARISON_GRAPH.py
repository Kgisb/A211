...
# User sees MTD / Cohort. We FLIP the internal logic as requested:
    # - Clicking "MTD"   → use Cohort logic internally
    # - Clicking "Cohort"→ use MTD logic internally
    mode_selection = st.radio(
        "Mode",
        ["MTD", "Cohort"],
        index=0,
        key="pva_mode",
        help="Choose how to read Actuals vs Plan.",
    )
    ...
    for i in range(int(rows_planned)):
        ...
        for label in extra_selected:
            ...
            if label in metric_defs and deals_hist > 0:
                try:
                    hist_events = int(
                        series.loc[
    ...
    try:
        snap = df.copy()
        # Build base masks using flipped internal `mode`
        if mode == "MTD":
            # Internal MTD: events in current month
            ...
            # Internal Cohort: deals created this month, outcomes anytime
        ...
        # Extra metrics for snapshot based on selection
        extra_cols = {}
        for label in extra_selected:
        ...
        grp = grp.rename(columns={deal_source_col: "JetLearn Deal Source", **agg_cols})

        # Attach Countries (Actual Snapshot) as comma-separated unique list from activity
        if country_col:
            mapping = {}
            for src in grp["JetLearn Deal Source"]:
                m_src = (df[deal_source_col].astype(str) == str(src)) & activity_mask
                ctrs = (
                    df.loc[m_src, country_col]
                    .dropna()
                    .astype(str)
                    .unique()
                )
        ...
        # Compute Conv% if possible
        if "Deals" in grp.columns and "Enrolments" in grp.columns:
            grp["Conv%"] = (
                (grp["Enrolments"] / grp["Deals"].where(grp["Deals"] > 0))
                .fillna(0) * 100
            )

        # Only show rows with some activity
        cols_show = [c for c in grp.columns if c not in []]
        grp = grp.loc[(grp.get("Deals", 0) > 0) | (grp.get("Enrolments", 0) > 0), cols_show]

        if not grp.empty:
            st.dataframe(
                grp.sort_values("JetLearn Deal Source"),
                use_container_width=True,
                hide_index=True,
            )
        else:
    ...
    if not plan_df.empty:
        for _, r in plan_df.iterrows():
            ...
            # Deals / Enrolments masks per (flipped) mode
            if mode == "MTD":
            ...
            # Start activity mask with deals/enrolments
            ...
            # Extra actual metrics + include into activity mask (so Countries (Actual) is basis actual)
            for label, (plan_key, actual_key, _, series) in metric_defs.items():
                if series is None:
                    continue
                if mode == "MTD":
            ...
            # Countries (Actual) strictly from actual activity_mask
            if country_col:
                ctrs = (
                    df.loc[activity_mask, country_col]
                    .dropna()
                    .astype(str)
                    .unique()
                )
    ...
    if not plan_df.empty and not actual_df.empty:
        ...
        # Simple visual on enrolments gap per source
        try:
            import altair as alt
            chart_data = merged[[
                "JetLearn Deal Source",
            ...
            pass
                ...
                # Metric comparison chart: choose what to compare (Plan vs Actual)
        try:
            metric_map = {}

            # Core metrics (ensure all required metrics are available if columns exist)
            if "Planned Deals" in merged.columns and "MTD/Cohort Deals (Actual)" in merged.columns:
                metric_map["Deals"] = ("Planned Deals", "MTD/Cohort Deals (Actual)")
            if "Planned Enrolments" in merged.columns and "MTD/Cohort Enrolments (Actual)" in merged.columns:
                metric_map["Enrolments"] = ("Planned Enrolments", "MTD/Cohort Enrolments (Actual)")
            if "Hist. Conv%" in merged.columns and "MTD/Cohort Conv% (Actual)" in merged.columns:
                metric_map["Conversion %"] = ("Hist. Conv%", "MTD/Cohort Conv% (Actual)")

            # Optional calibration metrics (if present in merged)
            if "Planned First Cal Count" in merged.columns and "Actual First Cal Count" in merged.columns:
                metric_map["First Calibration Count"] = ("Planned First Cal Count", "Actual First Cal Count")
            if "Planned Calib Resch Count" in merged.columns and "Actual Calib Resch Count" in merged.columns:
                metric_map["Calib Reschedule Count"] = ("Planned Calib Resch Count", "Actual Calib Resch Count")
            if "Planned Calib Done Count" in merged.columns and "Actual Calib Done Count" in merged.columns:
                metric_map["Calib Done Count"] = ("Planned Calib Done Count", "Actual Calib Done Count")
            ...
            if metric_map:
                ...
                chart_df["Type"] = chart_df["Type"].replace(ren_map)
            ...
            # Chart is helpful but not critical
            pass
        except Exception:
            # Chart is helpful but not critical
            pass
