...
def _render_marketing_plan_vs_actual(
    ...
    mode_selection = st.radio(
        ...
        index=0,
        key="pva_mode",
        help="MTD = month-to-date (event in current month); Cohort = deals created this month, outcomes anytime."
    )

    # Flip the underlying logic so existing calculations remain intact:
    # - When user clicks "MTD"  → use Cohort logic internally
    # - When user clicks "Cohort" → use MTD logic internally
    ...
    all_sources = sorted(
        s for s in df[deal_source_col].dropna().astype(str).unique()
    )
    all_countries = sorted(
        c for c in (df[country_col].dropna().astype(str).unique() if country_col else [])
    )
    ...
    for i in range(int(rows_planned)):
        ...
        # Planned extra metrics auto from historical ratios (basis lookback)
        extra_plan_vals = {}
        for label in extra_selected:
            plan_key, _, _, series = metric_defs[label]
            if series is not None and deals_hist > 0:
                try:
                    hist_events = int(
                        series.loc[
    ...
    # ---------- 2️⃣ Actuals ----------
    st.markdown("##### 2️⃣ Actual (driven by Mode)")

    # 2A Snapshot by Source x Country (independent view)
    try:
        if country_col:
            snap = df.copy()

            # Corrected semantics:
            # MTD  = deals & enrolments where dates fall in current month
            # Cohort = deals created in current month; enrolments can happen any time after
            if mode == "MTD":
                deal_mask = _create.between(month_start, month_end, inclusive="both")
                enrol_mask = _pay.between(month_start, month_end, inclusive="both")
            else:  # Cohort
                cohort_mask = _create.between(month_start, month_end, inclusive="both")
                deal_mask = cohort_mask
                enrol_mask = cohort_mask & _pay.notna()

            snap["is_deal"] = deal_mask
            snap["is_enrol"] = enrol_mask
            ...
            for label in extra_selected:
                ...
                if series is not None:
                    if mode == "MTD":
                        ...
                        m = cohort_mask & series.notna()
                    col_name = f"m_{actual_key}"
                    snap[col_name] = m
                    agg_map[col_name] = actual_key

            grp = (
                snap.groupby([deal_source_col, country_col], dropna=False)[list(agg_map.keys())]
                .sum()
                .reset_index()
            )
            ...
            if "Deals" in grp.columns:
                grp = grp[grp["Deals"] > 0]

            if not grp.empty and "Deals" in grp.columns and "Enrolments" in grp.columns:
                grp["Conv%"] = (
                    (grp["Enrolments"] / grp["Deals"].where(grp["Deals"] > 0)).fillna(0) * 100
                )

            if not grp.empty:
                st.write("**Actuals Snapshot (by Deal Source × Country)**")
                st.dataframe(
                    grp.sort_values(
        ...
        # soft fail; snapshot is not critical
        pass

    # 2B Actuals aligned to plan rows (for gap)
    actual_records = []
    if not plan_df.empty:
        for _, r in plan_df.iterrows():
            ...
            # Corrected semantics aligned with snapshot above
            if mode == "MTD":
            ...
            # Actual countries involved
            if mode == "MTD":
                any_mask = base_mask & (
                    _create.between(month_start, month_end, inclusive="both")
                    | _pay.between(month_start, month_end, inclusive="both")
                )
            else:
                any_mask = deals_mask
            ...
            if country_col:
                ...
                ctrs = []
            actual_countries = ", ".join(sorted(ctrs)) if len(ctrs) else "NA"

            rec = {
                "Row": int(r["Row"]),
                "JetLearn Deal Source": src,
                "Countries (Planned)": planned_c,
                "Countries (Actual)": actual_countries,
            ...
            # extra metrics actual for each plan row
            for label in extra_selected:
                ...
                if series is not None:
                    ...
                    # also keep planned already in r (plan_key)
    ...
    if not actual_df.empty:
        st.write("**Actuals vs Plan Rows**")
        st.dataframe(actual_df, use_container_width=True, hide_index=True)
    elif not plan_df.empty:
        st.info("No matching actuals found for the current mode / filters.")
    ...
    if not plan_df.empty and not actual_df.empty:
        ...
        if "Countries (Planned)" not in plan_gap.columns and "Countries" in plan_gap.columns:
            plan_gap = plan_gap.rename(columns={"Countries": "Countries (Planned)"})
        ...
        # place extra metric columns after core
        for label in extra_selected:
            plan_key, actual_key, gap_key, _ = metric_defs[label]
