...
# User sees MTD / Cohort. We FLIP the internal logic as requested:
    ...
    if not plan_df.empty and not actual_df.empty:
        ...
        try:
            # Build metric options only for metrics that exist in merged (so charts never silently fail)
            metric_map = {}
            ...
            # Calibration metrics (only if present)
            ...
            if metric_map:
                ...
                # Clean legend labels
                ren_map = {plan_col: "Plan", act_col: "Actual"}
                ...
                st.info("No comparable Plan vs Actual metrics available to plot.")
        except Exception:
